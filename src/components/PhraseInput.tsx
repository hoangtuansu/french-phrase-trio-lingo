...
const PhraseInput: React.FC<PhraseInputProps> = ({
  ...
  const toggleLanguage = (language: Language) => {
    const newSelection = selectedLanguages.includes(language) ? selectedLanguages.filter(l => l !== language) : [...selectedLanguages, language];
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 h-full flex flex-col">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-2">
              ...
              <Select value={sourceLanguage} onValueChange={value => onSourceLanguageChange(value as Language)}>
                ...
                <SelectContent>
                  {AVAILABLE_LANGUAGES.map(lang => <SelectItem key={lang.value} value={lang.value}>
                      {lang.label}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Tabs value={translationMode} onValueChange={value => onTranslationModeChange(value as TranslationMode)} className="w-full">
                <TabsList className="w-full grid grid-cols-3">
                  {TRANSLATION_MODES.map(mode => <TabsTrigger key={mode.value} value={mode.value} title={mode.description} className="text-xs">
                      {mode.label}
                    </TabsTrigger>)}
                </TabsList>
              </Tabs>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="w-full justify-between">
                  {selectedLanguages.length ? `${selectedLanguages.length} languages selected` : "Select languages"}
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent className="w-56">
                {AVAILABLE_LANGUAGES.map(lang => <DropdownMenuCheckboxItem key={lang.value} checked={selectedLanguages.includes(lang.value)} onCheckedChange={() => toggleLanguage(lang.value)}>
                    {lang.label}
          ...
          <div className="space-y-2 flex-grow">
            <div className="relative h-full flex flex-col">
              {pastedImage ? <div className="relative h-full flex flex-col">
                  <div className="flex-grow relative overflow-hidden border rounded-md p-2">
                    <img src={pastedImage} alt="Pasted content" className="max-w-full max-h-full object-contain mx-auto" />
                    <Button type="button" variant="ghost" size="sm" className="absolute top-2 right-2 h-6 w-6 p-0 bg-white/70 hover:bg-white/90" onClick={clearPastedImage}>
                      <X className="h-4 w-4" />
                      <span className="sr-only">Clear image</span>
                    </Button>
                  </div>
                </div> : <Textarea value={inputText} onChange={e => onInputTextChange(e.target.value)} onPaste={handlePaste} placeholder="Enter phrases or paste an image..." className="min-h-[150px] pr-10 flex-grow" />}
              
              {inputText && !pastedImage && <Button type="button" variant="ghost" size="sm" className="absolute top-2 right-2 h-6 w-6 p-0" onClick={clearPhrase}>
                  <X className="h-4 w-4" />
          ...
          <div className="flex gap-2">
            <Button type="button" variant="outline" className="flex-1" onClick={handleImageClick} disabled={isProcessingImage}>
              <ImageIcon className="h-4 w-4 mr-2" />
              {isProcessingImage ? "Processing..." : "Upload Image"}
            </Button>
            <Button type="submit" className="flex-1 bg-french-blue hover:bg-blue-800" disabled={isProcessingImage}>
              Translate
            </Button>
          </div>
      ...
      <TranslationResults translationResults={translationResults} selectedLanguages={selectedLanguages} translationMode={translationMode} />
      ...
      <Dialog open={isTextDialogOpen} onOpenChange={setIsTextDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <Textarea value={editedExtractedText} onChange={e => setEditedExtractedText(e.target.value)} className="min-h-[150px]" />
